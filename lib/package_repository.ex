defmodule Repository do
  ...
  def handle_call({:get_packages_for_files, files}, _, conn) do
    rows =
      execute_select(
        conn,
        "select packages.name, packages.path, files.name, files.path from packages inner join files on packages.name=files.package_name where files.name in (?1) sorted by package.name",
        [files]
      )
